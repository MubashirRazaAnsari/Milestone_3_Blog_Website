  ...
  return (  
    <>
      ...
      <section className='section_container'>
        <Image src={ideaImageUrl} alt={idea.ideaTitle} className='mx-auto !max-w-6xl max-h-[300px] rounded-xl' width={900} height={500} />


        <div className='space-y-5 mt-10 max-w-4xl mx-auto'>
          <div className='flex-between gap-5'>
            <Link href={`/user/${idea.author?._id}`} className='flex items-center mb-3 gap-2'>
              <Image src={authorImageUrl} alt={idea.author.name} className='drop-shadow-lg rounded-full' width={64} height={64} />
              <div>
                <p className='text-20-medium'>{idea.author.name}</p>
                <p className='text-16-medium !text-black-300'>@{idea.author.username}</p>
              </div>
            </Link>
          ...
          {parsedContent ? (<article className='prose max-w-4xl font-work-sans break-all' dangerouslySetInnerHTML={{__html: await parsedContent}} />) : (<p className='no-result'>No details available</p>)}
